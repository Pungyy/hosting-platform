import { NextResponse } from "next/server"
import { beforeEach, describe, expect, it, vi } from "vitest"

const {
  mockRequireSession,
  mockResolveListScope,
  mockQuery,
  mockCreateAgentDatabase,
  mockDeleteAgentDatabase,
  mockGetAgentDatabaseStatuses,
  mockEncryptSecret,
  mockReserveResourceQuota,
  mockReleaseResourceQuota,
} = vi.hoisted(() => ({
  mockRequireSession: vi.fn(),
  mockResolveListScope: vi.fn(),
  mockQuery: vi.fn(),
  mockCreateAgentDatabase: vi.fn(),
  mockDeleteAgentDatabase: vi.fn(),
  mockGetAgentDatabaseStatuses: vi.fn(),
  mockEncryptSecret: vi.fn(),
  mockReserveResourceQuota: vi.fn(),
  mockReleaseResourceQuota: vi.fn(),
}))

vi.mock("@/lib/auth/guard", () => ({
  requireSession: mockRequireSession,
}))

vi.mock("@/lib/auth/roles", () => ({
  resolveListScope: mockResolveListScope,
}))

vi.mock("@/lib/database", () => ({
  query: mockQuery,
}))

vi.mock("@/lib/agent/client", () => ({
  createAgentDatabase: mockCreateAgentDatabase,
  deleteAgentDatabase: mockDeleteAgentDatabase,
  getAgentDatabaseStatuses: mockGetAgentDatabaseStatuses,
}))

vi.mock("@/lib/agent/crypto", () => ({
  encryptSecret: mockEncryptSecret,
}))

vi.mock("@/lib/resources/quotas", () => ({
  MAX_DATABASES_PER_USER: 10,
  reserveResourceQuota: mockReserveResourceQuota,
  releaseResourceQuota: mockReleaseResourceQuota,
}))

import { POST } from "./route"

function sessionFor(role: "user" | "admin") {
  return {
    session: {
      session_id: "session-1",
      user_id: "user-1",
      email: "u@example.com",
      name: "Test User",
      role,
      expires_at: new Date(Date.now() + 3_600_000).toISOString(),
    },
    response: null,
  }
}

function quotaAvailable() {
  return { reserved: true, response: null }
}

function quotaExhausted() {
  return {
    reserved: false,
    response: NextResponse.json(
      {
        status: "error",
        message:
          "Le nombre maximal de bases de données par utilisateur (10) est atteint.",
      },
      { status: 409 },
    ),
  }
}

function postRequest(body: unknown) {
  return new Request("http://panel.local/api/databases", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  })
}

const AGENT_CREATED_DATABASE = {
  containerName: "hosting-db-test-db",
  image: "postgres:16-alpine",
  databaseName: "test_db",
  username: "test_db_user",
  password: "secret",
  port: 5432,
  running: true,
}

describe("POST /api/databases — quota par tenant (finding M3-1)", () => {
  beforeEach(() => {
    vi.resetAllMocks()
  })

  it("quota déjà atteint -> 409, l'Agent n'est jamais appelé, aucune requête DB", async () => {
    mockRequireSession.mockResolvedValue(sessionFor("user"))
    mockReserveResourceQuota.mockResolvedValue(quotaExhausted())

    const response = await POST(
      postRequest({ name: "test-db", engine: "postgres" }),
    )

    expect(response.status).toBe(409)
    expect(mockCreateAgentDatabase).not.toHaveBeenCalled()
    expect(mockQuery).not.toHaveBeenCalled()
    expect(mockReleaseResourceQuota).not.toHaveBeenCalled()
  })

  it("quota disponible -> création normale, aucune libération de quota", async () => {
    mockRequireSession.mockResolvedValue(sessionFor("user"))
    mockReserveResourceQuota.mockResolvedValue(quotaAvailable())
    mockEncryptSecret.mockReturnValue("iv:tag:ciphertext")

    mockQuery
      .mockResolvedValueOnce({ rows: [] }) // unicité du nom
      .mockResolvedValueOnce({ rows: [{ id: "server-1" }] }) // choix du serveur
      .mockResolvedValueOnce({
        rows: [
          {
            id: "db-1",
            name: "test-db",
            engine: "postgres",
            container_name: "hosting-db-test-db",
            container_id: null,
            image: "postgres:16-alpine",
            status: "online",
            database_name: "test_db",
            username: "test_db_user",
            internal_host: "hosting-db-test-db",
            internal_port: 5432,
            created_at: "2026-01-01T00:00:00.000Z",
            server_id: "server-1",
          },
        ],
      }) // INSERT databases

    mockCreateAgentDatabase.mockResolvedValue({
      database: AGENT_CREATED_DATABASE,
    })

    const response = await POST(
      postRequest({ name: "test-db", engine: "postgres" }),
    )

    expect(response.status).toBe(201)
    expect(mockReleaseResourceQuota).not.toHaveBeenCalled()
  })

  it("nom déjà pris après réservation -> quota libéré, 409", async () => {
    mockRequireSession.mockResolvedValue(sessionFor("user"))
    mockReserveResourceQuota.mockResolvedValue(quotaAvailable())
    mockQuery.mockResolvedValueOnce({ rows: [{ id: "existing-db" }] })

    const response = await POST(
      postRequest({ name: "test-db", engine: "postgres" }),
    )

    expect(response.status).toBe(409)
    expect(mockCreateAgentDatabase).not.toHaveBeenCalled()
    expect(mockReleaseResourceQuota).toHaveBeenCalledWith(
      "user-1",
      "database",
    )
  })

  it("serveur injoignable après réservation -> quota libéré, 503", async () => {
    mockRequireSession.mockResolvedValue(sessionFor("user"))
    mockReserveResourceQuota.mockResolvedValue(quotaAvailable())
    mockQuery
      .mockResolvedValueOnce({ rows: [] }) // unicité du nom
      .mockResolvedValueOnce({ rows: [] }) // aucun serveur joignable

    const response = await POST(
      postRequest({ name: "test-db", engine: "postgres" }),
    )

    expect(response.status).toBe(503)
    expect(mockCreateAgentDatabase).not.toHaveBeenCalled()
    expect(mockReleaseResourceQuota).toHaveBeenCalledWith(
      "user-1",
      "database",
    )
  })

  it("l'Agent échoue après réservation -> quota libéré, 502", async () => {
    mockRequireSession.mockResolvedValue(sessionFor("user"))
    mockReserveResourceQuota.mockResolvedValue(quotaAvailable())
    mockQuery
      .mockResolvedValueOnce({ rows: [] })
      .mockResolvedValueOnce({ rows: [{ id: "server-1" }] })

    mockCreateAgentDatabase.mockRejectedValue(
      new Error("Agent injoignable."),
    )

    const response = await POST(
      postRequest({ name: "test-db", engine: "postgres" }),
    )

    expect(response.status).toBe(502)
    expect(mockReleaseResourceQuota).toHaveBeenCalledWith(
      "user-1",
      "database",
    )
  })

  it(
    "échec d'insertion BDD après création Agent -> quota libéré ET " +
      "nettoyage du container orphelin",
    async () => {
      mockRequireSession.mockResolvedValue(sessionFor("user"))
      mockReserveResourceQuota.mockResolvedValue(quotaAvailable())
      mockEncryptSecret.mockReturnValue("iv:tag:ciphertext")
      mockQuery
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ id: "server-1" }] })
        .mockRejectedValueOnce(new Error("Contrainte violée."))

      mockCreateAgentDatabase.mockResolvedValue({
        database: AGENT_CREATED_DATABASE,
      })
      mockDeleteAgentDatabase.mockResolvedValue({ status: "ok" })

      const response = await POST(
        postRequest({ name: "test-db", engine: "postgres" }),
      )

      expect(response.status).toBe(500)
      expect(mockReleaseResourceQuota).toHaveBeenCalledWith(
        "user-1",
        "database",
      )
      expect(mockDeleteAgentDatabase).toHaveBeenCalledWith(
        "server-1",
        "test-db",
      )
    },
  )

  it("un admin créant sa propre base reste soumis au même quota (aucun contournement)", async () => {
    mockRequireSession.mockResolvedValue(sessionFor("admin"))
    mockReserveResourceQuota.mockResolvedValue(quotaExhausted())

    const response = await POST(
      postRequest({ name: "test-db", engine: "postgres" }),
    )

    expect(response.status).toBe(409)
    expect(mockCreateAgentDatabase).not.toHaveBeenCalled()
  })
})
